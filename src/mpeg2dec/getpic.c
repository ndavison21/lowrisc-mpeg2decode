...
static uint64_t saturate_time;
static uint64_t idct_time;
...
int bitstream_framenum, sequence_framenum;
{
  ...
  time_start();static uint64_t saturate_time;
  picture_data(bitstream_framenum);
  time_end("picture_data");

  printf("Saturate: %d mus\n", saturate_time);
  printf("IDCT: %d mus\n", idct_time);
...
int dct_type;
{
  ...
  for (comp = 0; comp < block_count; comp++) {
    ...
    if (acc_type & ACC_IDCT) {
      ...
      idct_time += time_get() - t - (t - t0);
    }
  }

  if (acc_type & ACC_IDCT) {
    acc_idct(ld->block[0], ld->block[0], 128 * block_count);
  }
